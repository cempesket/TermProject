jQuery(document).ready(function () {
    ...
    /*Creates an empty shopping cart in local storage */
    var cart = {}
    cart.quantity = 0;
    cart.products = [];
    ...
    function updateCart() {
        ...
        if (localStorage.getItem('cart') == null) {
            cartItems.text(parseInt("0"))
        }
        else {

            text = parseInt(JSON.parse(localStorage.getItem('cart')).quantity);
            cartItems.text(text);
        }
    }
    ...
    function initCustomization(items) {
        items.each(function () {
            ...
            addToCartBtn.on('click', function () {
                var product = {}
                var productAttr = $(this).closest("li");
                product.id = $(productAttr).attr('data-id');
                product.name = $(productAttr).attr('data-name');
                product.price = $(productAttr).attr('data-price');
                addToCart(product);
